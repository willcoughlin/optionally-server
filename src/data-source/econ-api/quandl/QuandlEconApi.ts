...
export default class QuandlEconApi extends RESTDataSource implements IEconApi {
  ...
  public async getNearestTBillRate(target: Moment) {
    ...
    const res = await this.get<QuandlErrorResponse | QuandlDatasetResponse>(`USTREASURY/BILLRATES.json?api_key=${this.apiKey}`);
  ...
  public async getInflationRate() {
    const res = await this.get<QuandlErrorResponse | QuandlDatasetResponse>(`RATEINF/INFLATION_USA.json?api_key=${this.apiKey}`);
